                ...
                <form wire:submit.prevent="save">
                    <div class="row mb-3">
                        <div class="col-md-4">
                            ...
                            @error('user.type') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    @if ($user->type)
                        <div class="row mb-3">
                        ...
                        @if ($user->type == 'student')
                            <div class="row mb-3">
                                <div class="col-md-4">
                                    ...
                                    @error('user.parent_phone') <span class="text-danger">{{ $message }}</span> @enderror
                                </div>
                                <div class="col-md">
                                    <label for="batch">Batch <span class="text-danger">*</span></label>
                                    <select class="form-control @error('user.batch_id') is-invalid @enderror" wire:model.defer="user.batch_id" id="batch">
                                        <option value="">- Select Batch -</option>
                                        @foreach ($batches as $batch)
                                            <option value="{{ $batch->id }}">{{ $batch->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('user.batch_id') <span class="text-danger">{{ $message }}</span> @enderror
                                </div>
                            </div>
                        @elseif($user->type == 'teacher')
                            <div class="row mb-3">
                                <div class="col-md-4">
                                    ...
                                    <input type="text" wire:model.defer="user.code_name" class="form-control @error('user.code_name') is-invalid @enderror">
                                    @error('user.code_name') <span class="text-danger">{{ $message }}</span> @enderror
                                </div>
                                <div class="col-md">
                                    <label for="subject_id">Subject <span class="text-danger">*</span></label>
                                    <select class="form-control @error('user.subject_id') is-invalid @enderror" wire:model.defer="user.subject_id" id="subject_id">
                                        <option value="">- Select Subject -</option>
                                        @foreach ($subjects as $subject)
                                            <option value="{{ $subject->id }}">{{ $subject->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('user.subject_id') <span class="text-danger">{{ $message }}</span> @enderror
                                </div>
                            </div>
                        @endif
                    @endif
                    ...
                    <div class="row mb-3">
                        <div class="col-12">
                            <button type="submit" wire:loading.attr="disabled" class="btn btn-success btn-sm">Submit <span
                                    wire:loading wire:target="save"> <i class="fa fa-spin fa-spinner"></i> </span></button>
                            <button type="button" wire:loading.attr="disabled" wire:click.prevent="cancel" class="btn btn-danger btn-sm">Cancel <span
                                    wire:loading wire:target="cancel"> <i class="fa fa-spin fa-spinner"></i>
                                </span></button>
                        </div>
                    </div>
                </form>
